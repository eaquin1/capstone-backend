...
class Meals {
    ...
    static async getMealsTimeRange(dexcom_id, startTime, endTime) {
        const mealsReponse = await db.query(
            `SELECT id, dexcom_id, foods, carb_count, date
            WHERE id=$1 AND date <=$2 AND date >=$3`, [dexcom_id, startTime, endTime]
        )
    ...
    static async createMeal(data){
        const result = await db.query(
            `INSERT INTO meals
            (dexcom_id, foods, carb_count, date)
            VALUES ($1, $2, $3, $4)
            RETURNING id, dexcom_id, foods, carb_count, date`,
            [data.dexcom_id, data.foods, data.carb_count, data.date]
        )
    }
    ...
    static async patchMeal(data){
        const result = await db.query(
            `UPDATE meals SET foods=$1 WHERE id=$2`, [data.foods, data.id]
        )
